...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor,
    ...
    :param block:  BaseQuantModel that needs to be optimized
    :param cali_data: data for calibration
    ...
    :param warmup: proportion of iter that
    :param act_quant: use activation quantization or not
    :param lr: learning rate for act delta learning
    :param p: L-p norm minimization
    :param multi_gpu: use multi-gpu
    :return:
    """
    ...
    if not act_quant:
        # replace weight qunatizer to adaRoundQuantizer
        for name, module in block.named_modules():
            if isinstance(module, QuantModule):
                ...
                module.weight_quantizer.soft_target = True
    ...
    loss_func = LossFunction(block, round_loss=loss_mode, weight=weight, max_count=iters, rec_loss=rec_loss, b_range=b_range, decay_start=0, warmup=warmup, p=p)

    # save data before optimizing the rounding
    cache_inps, cached_outs = save_input_output_data(model, block, cali_data, asym, act_quant, batch_size)
    if opt_mode != "mse":
    ...
    for i in range(iters):
        ...
        cur_inp = cache_inps[idx].to(device)
    ...
    # finish optimiation, use hard rounding
    for name, module in block.named_modules():
        if isinstance(module, QuantModule):
            module.weight_quantizer.soft_target = False
...
class LossFunction:
    ...
    rec_loss is the quadratic output reconstruction loss,
    round_loss is a regularization term to optimize the rounding policy


    """
    def __init__(self, block: BaseQuantBlock, round_loss: str = 'relaxation', weight: float = 1.,
                    rec_loss: str = 'mse', max_count: int = 2000, b_range: tuple = (10, 2), decay_start: float = 0.0, warmup: float = 0.0, p: float = 2.):
        self.block = block
    ...
    def __call__(self, pred, tgt, grad=None):
        ...
        if self.count < self.loss_start or self.round_loss == 'none':
            ...
            for name, module in self.block.named_modules():
                if isinstance(module, QuantModule):
                    round_value = module.weight_quantizer.get_soft_targets()
                    round_loss += self.weight * (1 - ((round_value - .5).abs() * 2).pow(b)).sum()
        else:
