...
class BaseQuantBlock(nn.Module):
    """
    Base implementation of block structures for all network.
    ...
    def set_quant_state(self, weight_quant: bool = False, act_quant: bool = False):
        # setting weight quantization and does not affect actual forward pass
        self.use_weight_quant = weight_quant
        self.use_weight_quant = act_quant
        for m in self.modules():
            if isinstance(m, QuantModule):
                m.set_quant_state(weight_quant,act_quant)


class QuanBasicBlock(BaseQuantBlock):
    """
...
class QuantBottleneck(BaseQuantBlock):
    ...
    """
    def __init__(self, bottlenect: Bottleneck, weight_quant_params: dict = {}, act_quant_params: dict = {}):
        super().__init__(act_quant_params)
        self.conv1 = QuantModule(bottlenect.conv1, weight_quant_params, act_quant_params)
        self.conv1.activation_function = bottlenect.relu1
        self.conv2 = QuantModule(bottlenect.conv2, weight_quant_params, act_quant_params)
        self.conv2.activation_function = bottlenect.relu2
        self.conv3 = QuantModule(bottlenect.conv3, weight_quant_params, act_quant_params, disable_act_quant=True)

        # modify the activation function to relu
        self.activation_function = bottlenect.relu3

        if bottlenect.downsample is None:
            self.downsample = None
        else:
            self.downsample = QuantModule(bottlenect.downsample[0], weight_quant_params, act_quant_params, disable_act_quant=True)

        #
        self.stride = bottlenect.stride
...
class QuantBottlenect(BaseQuantBlock):
    """
    Implementation of Quantized Bottleneck Block used in ResNet-50, -101 and -152.
    """
    ...
    def __init__(self, inv_res: InvertedResidual, weight_quant_params: dict = {}, act_quant_params: dict = {}):
        ...
        if self.expand_ratio == 1:
            self.conv = nn.Sequential(
                QuantModule(inv_res[0], weight_quant_params, act_quant_params),
                QuantModule(inv_res[3], weight_quant_params, act_quant_params, disable_act_quant=True),
            )
